...
class CylinderSimConfig:
    ...
    # ---- Output ----
    output_dir: str = "case/ComplexCylinderDIC"
...
def sample_radial_offset(theta: np.ndarray, y: np.ndarray, config: CylinderSimConfig) -> np.ndarray:
    """Deterministic sinusoidal radial offset on the unwrapped cylinder domain."""
    if config.surface_type == "cylinder":
        return np.zeros_like(theta, dtype=np.float64)
    if config.surface_type == "sinusoidal_cylinder":
        x_norm = theta / np.pi
        y_norm = 2.0 * y / config.cylinder_height
        A = config.sinusoidal_frequency
        B = config.sinusoidal_amplitude
        return B * np.sin(A * np.pi * x_norm) * np.sin(A * np.pi * y_norm)
    raise ValueError(f"Unknown surface_type: {config.surface_type}")


def sample_sinusoidal_radial_displacement(
    points: np.ndarray,
    config: CylinderSimConfig,
    amplitude: float,
) -> np.ndarray:
    """Radial displacement with the same sinusoidal product shape as the surface."""
    theta = np.arctan2(points[:, 2], points[:, 0])
    y = points[:, 1]
    x_norm = theta / np.pi
    y_norm = 2.0 * y / config.cylinder_height
    A = config.sinusoidal_frequency
    radial_delta = amplitude * np.sin(A * np.pi * x_norm) * np.sin(A * np.pi * y_norm)

    radial_dir = points.copy()
    radial_dir[:, 1] = 0.0
    nrm = np.linalg.norm(radial_dir, axis=1, keepdims=True)
    radial_dir = radial_dir / np.maximum(nrm, 1e-8)
    return radial_dir * radial_delta[:, None]
...
def generate_cylinder_surface(config: CylinderSimConfig, rng: np.random.Generator):
    ...
    # 3D positions of grain centers on the same surface geometry as sampled points.
    grain_offset = sample_radial_offset(grain_theta, grain_y, config)
    grain_r = R + grain_offset
    grain_x = grain_r * np.cos(grain_theta)
    grain_z = grain_r * np.sin(grain_theta)
    ...
    for block in range(num_blocks):
        ...
        # 3D coordinates on the configured cylinder surface.
        radial_offset = sample_radial_offset(theta, y_p, config)
        r_p = R + radial_offset
        x_p = r_p * np.cos(theta)
        z_p = r_p * np.sin(theta)
...
def apply_deformation(
    ...
    for step in range(1, config.num_deformed_steps + 1):
        ...
        elif config.deformation_type == "sinusoidal_radial":
            # Deterministic radial displacement with the same product-sine shape
            # as the reference surface perturbation. Default amplitude is 1 mm.
            disp = sample_sinusoidal_radial_displacement(
                pts,
                config,
                amplitude=config.deformation_magnitude * frac,
            )
            pts = pts + disp
...
def save_surface_visualizations(
    config: CylinderSimConfig,
    gt_dir: str,
    points_ref: np.ndarray,
):
    """Save unwrapped and 3D previews of the deterministic radial offset."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    except Exception as exc:
        print(f"  [WARN] Surface visualization skipped: {exc}")
        return

    n_grid = 241
    theta_grid = np.linspace(-np.pi, np.pi, n_grid)
    y_grid = np.linspace(-config.cylinder_height / 2.0, config.cylinder_height / 2.0, n_grid)
    theta_mesh, y_mesh = np.meshgrid(theta_grid, y_grid)
    delta_mesh = sample_radial_offset(theta_mesh, y_mesh, config)
    ...
    ax.set_xlabel("x_norm = theta / pi")
    ax.set_ylabel("y_norm = 2y / H")
    ax.set_title("Deterministic radial offset delta_r")
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label("delta_r (mm)")
    fig.tight_layout()
    fig.savefig(os.path.join(gt_dir, "radial_offset_unwrapped.png"))
    plt.close(fig)

    n_preview = min(50_000, len(points_ref))
    if n_preview < len(points_ref):
        preview_idx = np.linspace(0, len(points_ref) - 1, n_preview, dtype=np.int64)
        pts = points_ref[preview_idx]
    else:
        pts = points_ref
    theta = np.arctan2(pts[:, 2], pts[:, 0])
    y = pts[:, 1]
    radial_offset = sample_radial_offset(theta, y, config)
    ...
    ax.set_ylabel("Z (mm)")
    ax.set_zlabel("Y (mm)")
    ax.set_title("Sinusoidal cylinder point-cloud preview")
    try:
        ax.set_box_aspect((1, 1, config.cylinder_height / (2.0 * config.cylinder_radius)))
    except Exception:
        pass
    cbar = fig.colorbar(sc, ax=ax, shrink=0.72)
    cbar.set_label("delta_r (mm)")
    fig.tight_layout()
    fig.savefig(os.path.join(gt_dir, "surface_pointcloud_delta_r_preview.png"))
    plt.close(fig)
...
def save_ground_truth_outputs(
    ...
    theta_ref_gt = np.arctan2(points_ref_gt[:, 2], points_ref_gt[:, 0])
    y_ref_gt = points_ref_gt[:, 1]
    radial_offset_f32 = sample_radial_offset(theta_ref_gt, y_ref_gt, config).astype(np.float32)
    ...
    np.save(os.path.join(gt_dir, "theoretical_surface_radial_offset.npy"), radial_offset_f32)
    np.savez_compressed(
        os.path.join(gt_dir, "theoretical_surface_points.npz"),
        points=points_ref_f32,
        radial_offset=radial_offset_f32,
        surface_type=config.surface_type,
        cylinder_radius=config.cylinder_radius,
        cylinder_height=config.cylinder_height,
        sinusoidal_amplitude=config.sinusoidal_amplitude,
        sinusoidal_frequency=config.sinusoidal_frequency,
        units="mm",
    )
    ...
    meta = {
        ...
        "ground_truth_files": {
            ...
            "surface_radial_offset": "theoretical_surface_radial_offset.npy",
            ...
            "radial_offset_unwrapped_preview": "radial_offset_unwrapped.png",
            "surface_pointcloud_preview": "surface_pointcloud_delta_r_preview.png",
        },
    }
    with open(os.path.join(gt_dir, "meta.json"), "w") as f:
        json.dump(meta, f, indent=2)

    save_surface_visualizations(config, gt_dir, points_ref_gt)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="case/ComplexCylinderDIC")
    ...
    print(f"  Surface:        {config.surface_type}, B={config.sinusoidal_amplitude} mm, A={config.sinusoidal_frequency}")
