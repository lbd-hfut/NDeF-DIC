...
def run_sfm2world(config: Mapping[str, Any]) -> dict[str, str]:
    ...
    return run_chessboard_scale(
        ChessboardScaleConfig(
            ...
            scale_stat=str(sfm_cfg.get("scale_stat", "trimmed_mean")),
            trim_fraction=float(sfm_cfg.get("trim_fraction", 0.20)),
            max_pair_edge_cv=float(sfm_cfg.get("max_pair_edge_cv", 0.08)),
            max_edge_cv_warning=float(sfm_cfg.get("max_edge_cv_warning", 0.05)),
            corner_order_edge_cv_weight=float(sfm_cfg.get("corner_order_edge_cv_weight", 1.0)),
